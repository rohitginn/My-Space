...
export type CreateCommentInput = { x: number; y: number; content: string };
...
export async function createComment(userId: string, workspaceId: string, canvasId: string, input: CreateCommentInput) {
  ...
  const [created] = await db.insert(canvasComments).values({ canvasId, workspaceId, userId, userName: user.displayName, x: input.x, y: input.y, content: input.content }).returning();
...
// Compatibility handlers for the original comment-only routes.
export async function listCanvasComments(workspaceId: string, canvasId: string) {
  const rows = await db
    .select({ id: canvasComments.id, canvasId: canvasComments.canvasId, workspaceId: canvasComments.workspaceId, userId: canvasComments.userId, userName: canvasComments.userName, x: canvasComments.x, y: canvasComments.y, content: canvasComments.content, isResolved: canvasComments.isResolved, createdAt: canvasComments.createdAt })
    .from(canvasComments)
    .where(and(eq(canvasComments.workspaceId, workspaceId), eq(canvasComments.canvasId, canvasId)))
    .orderBy(asc(canvasComments.createdAt));
  return rows.map((row) => ({ ...row, x: Number(row.x), y: Number(row.y), isResolved: !!row.isResolved }));
}

export async function createCanvasComment(workspaceId: string, canvasId: string, userId: string, input: CreateCommentInput) {
  const [user] = await db.select({ displayName: users.displayName }).from(users).where(eq(users.id, userId)).limit(1);
  const [comment] = await db.insert(canvasComments).values({ workspaceId, canvasId, userId, userName: user?.displayName || 'Collaborator', x: input.x, y: input.y, content: input.content }).returning();
  return { ...comment, x: Number(comment.x), y: Number(comment.y), isResolved: !!comment.isResolved };
}
...
export async function toggleResolveComment(commentId: string) {
  ...
  const [updated] = await db.update(canvasComments).set({ isResolved: existing.isResolved ? null : new Date() }).where(eq(canvasComments.id, commentId)).returning();
  return { ...updated, x: Number(updated.x), y: Number(updated.y), isResolved: !!updated.isResolved };
}
