...
export default function Sidebar({ isMobileDrawer = false, onClose }: SidebarProps) {
  ...
  return <motion.aside animate={isMobileDrawer ? { width: 'min(84vw, 320px)' } : { width: collapsed ? 80 : 264 }} transition={{ type: 'spring', stiffness: 330, damping: 34 }} className="flex h-[100dvh] w-[min(84vw,320px)] shrink-0 flex-col justify-between border-r border-border bg-surface/90 py-5 backdrop-blur" style={activeWorkspace ? { borderColor: `${activeWorkspace.accentColor}55` } : undefined}>
    <div>
