...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-[100dvh]">
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-y-auto pt-14 md:pt-0"><AnimatePresence mode="wait"><motion.div key={pathname} {...pageTransition}>{children}</motion.div></AnimatePresence></main>
      <CommandPalette />
    </div>
  );
}
